...
def run_codex(
    ...
    output = _run_codex_responses(prompt, config, on_text=on_text)
...
def _run_codex_responses(prompt: str, config: AgentConfig, on_text: Callable[[str], None] | None = None) -> str:
    auth = _load_auth(config)
    payload = {
        "model": config.model,
    ...
    try:
        with urllib.request.urlopen(request, timeout=config.timeout_seconds) as response:
            return _consume_codex_stream(response, on_text=on_text)
    ...
    raise CodexUnavailable("Codex Responses request did not return data.")
...
def _consume_codex_stream(response, on_text: Callable[[str], None] | None = None) -> str:
    chunks: list[str] = []
    fallback: list[str] = []
